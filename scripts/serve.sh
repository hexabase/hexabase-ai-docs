...
if [ "$LANG" = "ja" ]; then
    ...
    echo "📖 Japanese Documentation: http://localhost:8099"
    mkdocs serve --config-file mkdocs.ja.yml --dev-addr 0.0.0.0:8099
elif [ "$LANG" = "en" ]; then
    echo "🇺🇸 Starting English development server..."
    echo "📖 English Documentation: http://localhost:8099"
    mkdocs serve --config-file mkdocs.yml --dev-addr 0.0.0.0:8099
else
